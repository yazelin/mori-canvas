...
> **主機設定的鎖**:公開部署請設 `ADMIN_TOKEN`(env)—— 設了之後 `POST /api/settings` 與「結束房間」要帶相符的 `X-Admin-Token` header,否則 401。沒設 token 時,whisperUrl / 處理模式 / 本機模式 / 主機 Groq key 這些主機級欄位只接受 loopback(127.0.0.1)來源修改,排列間距等個人偏好不受限;訪客在設定頁貼的 Groq key 只存自己瀏覽器、隨請求帶上(BYO),不會變成全 server 共用。
...
- **硬化 / 隱私 / 部署**:per-IP rate-limit(`DEMO_RATE_PER_MIN`)、贊助 banner、`.env` 設定、`LLM_LOCAL_ONLY=1`(開機鎖定本機模式:AI 只走本機 Ollama、雲端 STT 與訪客 BYO 端點一律封鎖、設定頁/API 關不掉)、`ADMIN_TOKEN`(鎖主機級設定與結束房間,需 `X-Admin-Token`;未設時主機級欄位僅限 loopback 修改)、Dockerfile + render.yaml + `deploy/` systemd/nginx 範例。
