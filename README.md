...
- **硬化 / 隱私 / 部署**:per-IP rate-limit(`DEMO_RATE_PER_MIN`)、贊助 banner、`.env` 設定、`LLM_LOCAL_ONLY=1`(開機鎖定本機模式:AI 只走本機 Ollama、雲端 STT 與訪客 BYO 端點一律封鎖、設定頁/API 關不掉)、Dockerfile + ghcr image(`ghcr.io/yazelin/mori-canvas`)+ render.yaml + `install.sh` 一鍵安裝 + `deploy/` systemd/nginx 範例。
