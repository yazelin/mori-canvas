...
```
  ...
  ┌────────────────────────────────────────┐
  │  Rust binary :5174 (一顆,全包)         │◀──── 手機 / 同事的筆電
  │   ├─ 內嵌前端(/)                        │      (開瀏覽器連這台,零安裝)
  │   ├─ /sync(ws)即時同步 (yrs CRDT)      │
  │   ├─ /api STT → mori-ear / Groq Whisper │  ← 你的 mori-ear 或自訂
  │   └─ /api agent(意圖判斷 + 整理/執行)   │  ← 你的 Groq key 或訪客自帶
...
- **會議紀錄匯出**:**HTML**(AI 摘要 + 逐字稿,雙擊就能讀)/ MD / PNG;**畫板存檔(.json)** 可完整還原、傳給別人接著編。
