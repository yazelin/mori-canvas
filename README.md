...
**[立即試玩](https://mori-canvas.onrender.com/) ・ [先看示範板](https://mori-canvas.onrender.com/?room=DEMO) ・ [使用手冊](https://yazelin.github.io/mori-canvas/) ・ 全 MIT、自架零授權成本**
...
### 1) 試玩(免裝,點連結就玩)
社群試玩版部署在 Render:**`https://<你的-render-網址>.onrender.com`**(部署好後把這行換成實際網址)。朋友點開掃 QR 就進,什麼都不用裝。只想花十秒看「長好的板」長什麼樣,直接開**示範板** [https://mori-canvas.onrender.com/?room=DEMO](https://mori-canvas.onrender.com/?room=DEMO) (每小時自動重置)。AI 走站長的 Groq key(有 per-IP 限流);朋友想用自己的額度可在 **⚙ 設定 → 用你自己的 AI** 填任何 OpenAI 相容的 base/key/model,或直接貼逐字稿。

**自己部署社群版到 Render(GitHub-driven):**
1. render.com → 用 GitHub 登入 → **New + → Blueprint → 選這個 repo**(自動讀 `render.yaml`)。
2. 服務 **Environment → 填 `GROQ_API_KEY`**(設了花費上限的 key)與 **`ADMIN_TOKEN`**(任意長隨機字串;設了之後改主機設定/結束房間要帶 `X-Admin-Token` header,訪客就改不動 whisperUrl、模式、本機模式這些主機級設定)→ Deploy。
3. 之後每次 `git push`,Render 自動重部署(`render.yaml` 設了 `autoDeploy`)。
> 免費方案閒置 15 分鐘會休眠(首次再進來等 ~30-60 秒冷啟動);要常開可換付費或自架。

### 2) 自己跑 server(給團隊、想自架)

**最快:Docker 一行**(image 打 `v*` tag 時由 CI 發佈到 ghcr):
```bash
docker run -p 1334:1334 -v "$PWD/data:/app/.data" -e GROQ_API_KEY=gsk_xxx ghcr.io/yazelin/mori-canvas
```
開 `http://localhost:1334/`;白板資料持久化在 `./data`(沒掛 volume 的話容器一刪就沒)。
...
```
裝到 `~/.local/share/mori-canvas/`、指令 symlink 在 `~/.local/bin/`;重跑即升級。macOS / Windows 直接到 [Releases](https://github.com/yazelin/mori-canvas/releases) 抓對應的 server 包。

**從源碼 build**(備案;需要 Rust + Node):
```bash
...
```
AI 自己準備一把:`~/.mori/config.json` 或 `.env` 的 `GROQ_API_KEY`、設定頁的 BYO、或本機 Ollama。區網 HTTPS 版見上面「公司區網版」;要常駐 + 正式 TLS,用 [`deploy/mori-canvas.service`](deploy/mori-canvas.service)(systemd)+ [`deploy/nginx.conf.example`](deploy/nginx.conf.example)(反向代理),照檔頭註解換路徑、env 填進 `/etc/mori-canvas.env` 即可。
> **主機設定的鎖**:公開部署請設 `ADMIN_TOKEN`(env)—— 設了之後 `POST /api/settings` 與「結束房間」要帶相符的 `X-Admin-Token` header,否則 401。沒設 token 時,whisperUrl / 處理模式 / 本機模式 / 主機 Groq key 這些主機級欄位只接受 loopback(127.0.0.1)來源修改,排列間距等個人偏好不受限;訪客在設定頁貼的 Groq key 只存自己瀏覽器、隨請求帶上(BYO),不會變成全 server 共用。注意:**同主機的反向代理**(上面的 nginx 範例)會讓所有訪客的來源看起來都是 loopback,這種部署一定要設 `ADMIN_TOKEN`。
...
- **房間管理**:分享面板可看進行中房間、進入別房、**結束此房**(清空)。房間落地保留(`.data/<房號>.bin`),沒人連也在、重啟還原;自架預設**無過期**,設 `ROOM_TTL_HOURS` 可讓閒置房自動清掉(demo 站設 72h)、`MAX_ROOMS` 限制總房數、`PUBLIC_ROOM_LIST=0` 讓房間清單只回數量不洩漏房號(demo 站預設關閉清單)。詳見 `.env.example`。
- **示範板**:`?room=DEMO` 是常駐示範房 — server 啟動時自動種出一份做好的會議板,**每小時重置回種子內容**(隨便塗沒關係);空白板上也有「用示範逐字稿試試」一鍵看 AI 整理上板。
...
- **硬化 / 隱私 / 部署**:per-IP rate-limit(`DEMO_RATE_PER_MIN`)、贊助 banner、`.env` 設定、`LLM_LOCAL_ONLY=1`(開機鎖定本機模式:AI 只走本機 Ollama、雲端 STT 與訪客 BYO 端點一律封鎖、設定頁/API 關不掉)、`ADMIN_TOKEN`(鎖主機級設定與結束房間,需 `X-Admin-Token`;未設時主機級欄位僅限 loopback 修改)、Dockerfile + ghcr image(`ghcr.io/yazelin/mori-canvas`)+ render.yaml + `install.sh` 一鍵安裝 + `deploy/` systemd/nginx 範例。
